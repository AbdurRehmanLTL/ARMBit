...
const resp = await cache.match('/data/cache/ARMBit-ClientDb.db');
...
```bash
dotnet ef migrations add InitialMigration --context OfflineDbContext --output-dir Data/Migrations --project ../Client/ARMBit.Client.Core/ARMBit.Client.Core.csproj
```
...
1. Make sure `ARMBit.Server` is set as the default startup project, and `ARMBit.Client.Core` is the default project in the Package Manager Console.

2. Run the following command:

    ```powershell
    Optimize-DbContext -Context OfflineDbContext -OutputDir Data/CompiledModel -Namespace ARMBit.Client.Core.Data
    ```
